...
object UIUtils {
    ...
    @JvmStatic
    fun getDisplayDetailsTitle(info: MovieModel?): String = "${info?.title}(${getDisplayYearOfDate(info?.release_date)})"

    @JvmStatic
    fun getDisplayDetailsSubTitle(movie: MovieModel?): String {
        var genres = ""
    ...
    fun getDisplayYearOfDate(strDate: String?): String =
        if (strDate.isNullOrBlank()) "N/A" else "${LocalDate.parse(strDate).year}"
}